...
(define ($include m) (eval (read (open-input-file m))))
(define ($oword x) (list 8 x))
(define ($qword x) (list 4 x))
(define ($dword x) (list 2 x))
(define ($word x) (list 1 x))
(define l_refs (make-hash-table))
(define v_refs (make-hash-table))
(define ($def_label)
  (define lref li)
  (set! li (+ li 1))
  (if (= pass 0)
    lref
    ($qword (hash-table-ref l_refs lref))
  )
)
(define ($label lref)
  (if (= pass 0)
